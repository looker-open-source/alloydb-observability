view: navigation_bar {
  ...
  derived_table: {
    sql: SELECT CURRENT_DATE as data_date ;;
  }
  ...
  dimension: horizontal_navigation_bar {
    ...
    type: string
    # Fixed for PostgreSQL dialect (single quotes for string literal)
    sql: '' ;;
    html:
    <div style="background: #ffffff; font-family: 'Google Sans', Arial, sans-serif; border-bottom: 1px solid #e0e0e0; padding: 12px; display: flex; align-items: center; justify-content: flex-start; gap: 15px;">
      ...
      {% else %}
        <a style="{{ style_inactive }}" href="/dashboards/operational_intelligence_alloy_db::alloydb_performance_overview?Database+Name={{ _filters['pg_stat_database.datname'] | url_encode }}">📊 Executive Health</a>
      {% endif %}
      ...
      {% else %}
        <a style="{{ style_inactive }}" href="/dashboards/operational_intelligence_alloy_db::bottleneck_analysis?Database+Name={{ _filters['pg_stat_database.datname'] | url_encode }}">🔍 Bottleneck Analysis</a>
      {% endif %}
      ...
      {% else %}
        <a style="{{ style_inactive }}" href="/dashboards/operational_intelligence_alloy_db::looker_impact?Database+Name={{ _filters['pg_stat_database.datname'] | url_encode }}">🎯 Looker Impact</a>
      {% endif %}
